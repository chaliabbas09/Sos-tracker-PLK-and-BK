...
var UIController = (function () {
  ...
  var kfcServiceTimes = {
    OTD_Time: 135000, //time in miliseconds = 60 seconds --> Im changing this to the Popeyes Dine in times now (2:15)
    Total_time: 135000, //time in miliseconds = 135 (2:15 minutes) seconds ---> Popeyes total time 2:15
    Total_DT_time: 150000, // Drive thru time in miliseconds = 60 (1 min) seconds - --> Popeyes service time 3:15
    Total_HD: 29000, // home delivery time in miliseconds = 29 seconds  --> This is Tim Hortons max time of extraction time,
    Total_HD_min: 23000, // 23 seconds  --> This is Tim Hortons min extraction time
    Total_BK_target: 120000, // Burger time in miliseconds = 120 (2 min 00 seconds) seconds => new time for BK front counter
    Max_BK_target: 180000, // Burger King time  max in miliseconds = 180 (3 min 00 seconds) seconds => new max time for Burger King
    Max_PLK_target: 195000, // PLK time  max in miliseconds = 195 (3 min 15 seconds) seconds => new max time for Popeyes
    Max_TH_target: 240000 // Tim Hortons time  max in miliseconds = 240 (4 min 00 seconds) seconds => new max time for Tim Hortons
    };

    var calculateSOSTargetTime = function(type) {
      var SOSTargetTime, bk_max, th_min_time, plk_max, th_max;

      if(type === 'kfc') {
        SOSTargetTime = kfcServiceTimes.Total_time;
        plk_max = kfcServiceTimes.Max_PLK_target;
      } else if(type === 'kfc_DT') {
        SOSTargetTime = kfcServiceTimes.Total_DT_time;
        th_max = kfcServiceTimes.Max_TH_target;
      } else if(type === 'kfc_HD') {
        SOSTargetTime = kfcServiceTimes.Total_HD;
        th_min_time = kfcServiceTimes.Total_HD_min;
      } else if(type === 'bk') {
...
// it checks if difference between actual speed and set speed is more than 10 seconds (1 sec = 1000 miliseconds)
  ...
  serviceTime_min     = calculateSOSTargetTime(sosType).th_min_time;
  serviceTime_max_PLK = calculateSOSTargetTime(sosType).plk_max;
  serviceTime_max_TH  = calculateSOSTargetTime(sosType).th_max;
  totalCustomers      = obj.customers;   // <- total number of completed customers

  avgOTD = this.convertToTime(obj.averageOTD).minutes + ':' +
           this.convertToTime(obj.averageOTD).seconds;

  avgService = this.convertToTime(obj.averageService).minutes + ':' +
               this.convertToTime(obj.averageService).seconds;

  // if sosType is different than Pizza Hut, check the average Order to delivery time
  if (sosType !== 'ph') {
    if (avgOTD) {
      element = sosType + '-' +  DOMStrings.serviceTime;
  ...
  // if sosType is different than Pizza Hut, check the average Service time
  if (sosType !== 'ph') {
    if (avgService) {
      element = sosType + '-' +  DOMStrings.totalServiceTime;
      ...
      if (sosType === 'bk') {
        ...
        if (obj.averageService > serviceTime_max) {
          ...
          obj.averageService > serviceTime_target &&
          obj.averageService < serviceTime_max
        ) {
        ...
        // KFC average: green → orange → red (plk_max)
        if (obj.averageService > serviceTime_max_PLK) {
          this.changeBackground_offTarget(elementOTD);

        } else if (
          obj.averageService > serviceTime_target &&
          obj.averageService <= serviceTime_max_PLK
        ) {
          this.changeBackground_betweenTarget(elementOTD);
      ...
      } else if (sosType === 'kfc_DT') {

        // KFC DT average: green → orange → red (TH_max)
        if (obj.averageService > serviceTime_max_TH) {
          this.changeBackground_offTarget(elementOTD);

        } else if (
          obj.averageService > serviceTime_target &&
          obj.averageService <= serviceTime_max_TH
        ) {
          this.changeBackground_betweenTarget(elementOTD);
      ...
      } else if (sosType === 'kfc_HD') {

        // HD average: OFF target if too fast OR too slow
        if (
          obj.averageService < serviceTime_min ||
          obj.averageService > serviceTime_target
        ) {
          this.changeBackground_offTarget(elementOTD);
        } else {
    ...
    // ===============================
    // GS53C escalation check (DEBUG)
    // avgService is in MILLISECONDS
    // Trigger at 10s (10000 ms) & min 5 customers
    // ===============================
    if (sosType === 'bk' || sosType === 'kfc' || sosType === 'kfc_DT') {
      var avgMs = obj.averageService;
      ...
      var alertIdMap = {
        bk: 'bk-escalation-alert',
        kfc: 'kfc-escalation-alert',
        kfc_DT: 'kfc_DT-escalation-alert'
      };

      var alertEl = document.getElementById(alertIdMap[sosType]);

      if (alertEl) {
        if (avgMs >= 301000 && totalCustomers >= 5) {   // DEBUG: 10 seconds
  ...
  // ---- finish time + per-guest colouring ----
  if (finishTimeCalc) {
    ...
    calcOTD = this.convertToTime(obj.OTDTime).minutes + ':' +
              this.convertToTime(obj.OTDTime).seconds;
    calcTotServTime = this.convertToTime(obj.serviceTime).minutes + ':' +
                      this.convertToTime(obj.serviceTime).seconds;
    ...
    if (sosType === 'bk') {
      ...
      if (obj.serviceTime > serviceTime_max) {
        ...
        obj.serviceTime > serviceTime_target &&
        obj.serviceTime < serviceTime_max
      ) {
      ...
      // KFC: green → orange → red (using plk_max)
      if (obj.serviceTime > serviceTime_max_PLK) {
        this.changeBackground_offTarget(elementTotalTime);

      } else if (
        obj.serviceTime > serviceTime_target &&
        obj.serviceTime <= serviceTime_max_PLK
      ) {
        this.changeBackground_betweenTarget(elementTotalTime);
      ...
      // KFC DT: green → orange → red (using TH_max)
      if (obj.serviceTime > serviceTime_max_TH) {
        this.changeBackground_offTarget(elementTotalTime);

      } else if (
        obj.serviceTime > serviceTime_target &&
        obj.serviceTime <= serviceTime_max_TH
      ) {
        this.changeBackground_betweenTarget(elementTotalTime);
      ...
      // HD: OFF target if too fast OR too slow
      if (
        obj.serviceTime < serviceTime_min ||
        obj.serviceTime > serviceTime_target
      ) {
        this.changeBackground_offTarget(elementTotalTime);
      } else {
...
var controller = (function (timeCtrl, UICtrl) {
  ...
  var updateServiceTimes = function (id, type) {
    ...
    // Display the service times on the UI
